...
class Converter:
    ...
    def pdf2csv(self, file_url):
        ...
        self.names = ['classNbr', 'subj', 'crs', 'courseTitle', 'sbc', 'cmp', 'sctn', 'credits', 'day', 'startTime', 'endTime', 'room', 'instructor', 'likes', 'reviews', 'instructor_names', 'semesters']
    ...
    def courses2db(self, reset_db=True): 
        ...
        for i in range(len(self.df)):
            ...
            if find_it != None and self.df[self.category[1]].iloc[i] != 'Subj':
                find_it[self.names[8]].append(self.df[self.category[8]].iloc[i])
                find_it[self.names[9]].append(self.df[self.category[9]].iloc[i])
                find_it[self.names[10]].append(self.df[self.category[10]].iloc[i])
                find_it[self.names[11]].append(self.df[self.category[11]].iloc[i])
                find_it[self.names[12]].append(self.df[self.category[12]].iloc[i])
                find_it[self.names[15]] += ', ' + str(self.df[self.category[12]].iloc[i])
                find_it[self.names[16]].append(self.semester)
                ...
                self.collection.find_one_and_update({self.names[1]: self.df[self.category[1]].iloc[i],
                                            ...
                                            self.names[16]: find_it[self.names[16]]}
                                            })
            ...
            # 수업 정보 없으면 새로 만듦
                ...
                self.names[8]: [self.df[self.category[8]].iloc[i]],
                self.names[9]: [self.df[self.category[9]].iloc[i]],
                self.names[10]: [self.df[self.category[10]].iloc[i]],
                self.names[11]: [self.df[self.category[11]].iloc[i]],
                self.names[12]: [self.df[self.category[12]].iloc[i]],
                self.names[13]: [],
                self.names[14]: [],
                self.names[15]: str(self.df[self.category[12]].iloc[i]),
                self.names[16]: [self.semester]
